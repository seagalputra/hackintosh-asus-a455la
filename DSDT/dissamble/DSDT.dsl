 ...
 *
 * Disassembly of DSDT.aml, Thu Apr 25 13:35:44 2019
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x00018C2D (101421)
 *     Revision         0x02
 *     Checksum         0x40
 *     OEM ID           "_ASUS_"
...
DefinitionBlock ("", "DSDT", 2, "_ASUS_", "Notebook", 0x01072009)
{
    ...
    Name (VGAF, One)
    OperationRegion (GNVS, SystemMemory, 0xD97BE000, 0x0359)
    ...
    Scope (_SB)
    {
        Device (PCI0)
        {
            ...
            Device (LPCB)
            {
                ...
                OperationRegion (CPSB, SystemMemory, 0xD8ACBF18, 0x10)
...
RWAK (Arg0)
    ...
    Scope (_SB.PCI0.I2C1)
    {
        Device (ACD0)
        {
            ...
            Name (_SUB, "170D1043")  // _SUB: Subsystem ID
            ...
            Method (WMNB, 3, Serialized)
            {
                ...
                If (LEqual (Local0, 0x53545344))
                {
                    ...
                    If (LEqual (IIA0, 0x00060062))
                    {
                        ...
                        Else
                        {
                            Store (0x00010001, Local0)
                        }

                        Return (Local0)
                    }

                    If (LEqual (IIA0, 0x00100053))
                    {
                        Store (0x00010001, Local0)
                        Return (Local0)
                    }
                    ...
                    If (LEqual (IIA0, 0x00120063))
                    {
                        Return (^^PCI0.LPCB.EC0.DKST ())
                    }

                    If (LEqual (IIA0, 0x00060071))
                    {
                        Return (^^PCI0.LPCB.EC0.PHST ())
                    }

                    If (LEqual (IIA0, 0x00020027))
                    {
                        Return (^^PCI0.LPCB.EC0.SCST ())
                    }
                }
...
Else
{
    ...
    Scope (_SB.PCI0.I2C0)
    {
        Device (SHUB)
        {
            ...
            Name (_CID, "PNP0C50")  // _CID: Compatible ID
            Name (_UID, One)  // _UID: Unique ID
            Name (_HID, "INVN6500")  // _HID: Hardware ID
            Method (_STA, 0, NotSerialized)  // _STA: Status
            {
                Return (0x0F)
            }
            ...
            Name (BUF0, ResourceTemplate()
{
    ...
    OperationRegion (ABNV, SystemMemory, 0xD97AD000, 0x000D)
